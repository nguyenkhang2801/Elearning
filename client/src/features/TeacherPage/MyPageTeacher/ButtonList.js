...
function ButtonList(props) {
  ...
  useEffect(() => {
    Axios.get(`http://localhost:3001/api/getalltextbook`).then((response) => {
  ...
  function handleStudentList() {
    ...
    Axios.get(
      `http://localhost:3001/api/getstudentinclass?classId=${teacherClass.ClassId}`
    ).then((response) => {
        ...
        <FormControl className={classes.formControl} error>
          ...
          >
            <optgroup label="Textbooks">
              {allTextBook.map((book) => (
                <option value={book.TextBookName}>{book.TextBookName}</option>
              ))}
            </optgroup>
          </NativeSelect>
          <Button
  ...
  function handleAdd(bookName) {
    const bookAdding = allTextBook.filter(
      (book) => book.TextBookName === bookName
    )[0];
  ...
  return (
    <Box>
      <h2>
        {teacherClass.SubjectName} - {teacherClass.CRoom}
        {teacherClass.CBuilding}{" "}
      </h2>
      <Button
        variant="contained"
        color="secondary"
      ...
      <Dialog
        ...
        aria-labelledby="form-dialog-title"
      >
        <DialogTitle id="form-dialog-title">Main TextBook</DialogTitle>
        <DialogContent>
